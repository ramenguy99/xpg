...
struct CommandBuffer: GfxObject {
    ...
    void copy_buffer_range(const Buffer& src, const Buffer& dst, VkDeviceSize size, VkDeviceSize src_offset, VkDeviceSize dst_offset) {
        if (src_offset + size > src.size) {
            nb::raise("Source buffer too small. Src size: %zu. Src offset: %" PRIu64 ". Size: %" PRIu64, src.size, src_offset, size);
        }
        if (dst_offset + size > dst.size) {
            nb::raise("Dest buffer too small. Dst size: %zu. Dst offset: %" PRIu64 ". Size: %" PRIu64, dst.size, dst_offset, size);
        }
    ...
    void copy_image_to_buffer_range(
        ...
        gfx::CmdCopyImageToBuffer(buffer, {
            ...
            .image_aspect = (VkImageAspectFlags)image_aspect_mask,
            .image_mip = image_mip,
...
void gfx_create_bindings(nb::module_& m)
{
    ...
    nb::enum_<VkBlendFactor>(m, "BlendFactor")
        .value("ZERO",                     VK_BLEND_FACTOR_ZERO)
        .value("ONE",                      VK_BLEND_FACTOR_ONE)
        .value("SRC_COLOR",                VK_BLEND_FACTOR_SRC_COLOR)
        .value("ONE_MINUS_SRC_COLOR",      VK_BLEND_FACTOR_ONE_MINUS_SRC_COLOR)
        .value("DST_COLOR",                VK_BLEND_FACTOR_DST_COLOR)
        .value("ONE_MINUS_DST_COLOR",      VK_BLEND_FACTOR_ONE_MINUS_DST_COLOR)
        .value("SRC_ALPHA",                VK_BLEND_FACTOR_SRC_ALPHA)
        .value("ONE_MINUS_SRC_ALPHA",      VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA)
        .value("DST_ALPHA",                VK_BLEND_FACTOR_DST_ALPHA)
        .value("ONE_MINUS_DST_ALPHA",      VK_BLEND_FACTOR_ONE_MINUS_DST_ALPHA)
        .value("CONSTANT_COLOR",           VK_BLEND_FACTOR_CONSTANT_COLOR)
        .value("ONE_MINUS_CONSTANT_COLOR", VK_BLEND_FACTOR_ONE_MINUS_CONSTANT_COLOR)
        .value("CONSTANT_ALPHA",           VK_BLEND_FACTOR_CONSTANT_ALPHA)
        .value("ONE_MINUS_CONSTANT_ALPHA", VK_BLEND_FACTOR_ONE_MINUS_CONSTANT_ALPHA)
        .value("SRC_ALPHA_SATURATE",       VK_BLEND_FACTOR_SRC_ALPHA_SATURATE)
        .value("SRC1_COLOR",               VK_BLEND_FACTOR_SRC1_COLOR)
        .value("ONE_MINUS_SRC1_COLOR",     VK_BLEND_FACTOR_ONE_MINUS_SRC1_COLOR)
        .value("SRC1_ALPHA",               VK_BLEND_FACTOR_SRC1_ALPHA)
        .value("ONE_MINUS_SRC1_ALPHA",     VK_BLEND_FACTOR_ONE_MINUS_SRC1_ALPHA)
    ;

    nb::enum_<VkBlendOp>(m, "BlendOp")
        .value("ADD",                VK_BLEND_OP_ADD)
        .value("SUBTRACT",           VK_BLEND_OP_SUBTRACT)
        .value("REVERSE_SUBTRACT",   VK_BLEND_OP_REVERSE_SUBTRACT)
        .value("MIN",                VK_BLEND_OP_MIN)
        .value("MAX",                VK_BLEND_OP_MAX)
        .value("ZERO",               VK_BLEND_OP_ZERO_EXT)
        .value("SRC",                VK_BLEND_OP_SRC_EXT)
        .value("DST",                VK_BLEND_OP_DST_EXT)
        .value("SRC_OVER",           VK_BLEND_OP_SRC_OVER_EXT)
        .value("DST_OVER",           VK_BLEND_OP_DST_OVER_EXT)
        .value("SRC_IN",             VK_BLEND_OP_SRC_IN_EXT)
        .value("DST_IN",             VK_BLEND_OP_DST_IN_EXT)
        .value("SRC_OUT",            VK_BLEND_OP_SRC_OUT_EXT)
        .value("DST_OUT",            VK_BLEND_OP_DST_OUT_EXT)
        .value("SRC_ATOP",           VK_BLEND_OP_SRC_ATOP_EXT)
        .value("DST_ATOP",           VK_BLEND_OP_DST_ATOP_EXT)
        .value("XOR",                VK_BLEND_OP_XOR_EXT)
        .value("MULTIPLY",           VK_BLEND_OP_MULTIPLY_EXT)
        .value("SCREEN",             VK_BLEND_OP_SCREEN_EXT)
        .value("OVERLAY",            VK_BLEND_OP_OVERLAY_EXT)
        .value("DARKEN",             VK_BLEND_OP_DARKEN_EXT)
        .value("LIGHTEN",            VK_BLEND_OP_LIGHTEN_EXT)
        .value("COLORDODGE",         VK_BLEND_OP_COLORDODGE_EXT)
        .value("COLORBURN",          VK_BLEND_OP_COLORBURN_EXT)
        .value("HARDLIGHT",          VK_BLEND_OP_HARDLIGHT_EXT)
        .value("SOFTLIGHT",          VK_BLEND_OP_SOFTLIGHT_EXT)
        .value("DIFFERENCE",         VK_BLEND_OP_DIFFERENCE_EXT)
        .value("EXCLUSION",          VK_BLEND_OP_EXCLUSION_EXT)
        .value("INVERT",             VK_BLEND_OP_INVERT_EXT)
        .value("INVERT_RGB",         VK_BLEND_OP_INVERT_RGB_EXT)
        .value("LINEARDODGE",        VK_BLEND_OP_LINEARDODGE_EXT)
        .value("LINEARBURN",         VK_BLEND_OP_LINEARBURN_EXT)
        .value("VIVIDLIGHT",         VK_BLEND_OP_VIVIDLIGHT_EXT)
        .value("LINEARLIGHT",        VK_BLEND_OP_LINEARLIGHT_EXT)
        .value("PINLIGHT",           VK_BLEND_OP_PINLIGHT_EXT)
        .value("HARDMIX",            VK_BLEND_OP_HARDMIX_EXT)
        .value("HSL_HUE",            VK_BLEND_OP_HSL_HUE_EXT)
        .value("HSL_SATURATION",     VK_BLEND_OP_HSL_SATURATION_EXT)
        .value("HSL_COLOR",          VK_BLEND_OP_HSL_COLOR_EXT)
        .value("HSL_LUMINOSITY",     VK_BLEND_OP_HSL_LUMINOSITY_EXT)
        .value("PLUS",               VK_BLEND_OP_PLUS_EXT)
        .value("PLUS_CLAMPED",       VK_BLEND_OP_PLUS_CLAMPED_EXT)
        .value("PLUS_CLAMPED_ALPHA", VK_BLEND_OP_PLUS_CLAMPED_ALPHA_EXT)
        .value("PLUS_DARKER",        VK_BLEND_OP_PLUS_DARKER_EXT)
        .value("MINUS",              VK_BLEND_OP_MINUS_EXT)
        .value("MINUS_CLAMPED",      VK_BLEND_OP_MINUS_CLAMPED_EXT)
        .value("CONTRAST",           VK_BLEND_OP_CONTRAST_EXT)
        .value("INVERT_OVG",         VK_BLEND_OP_INVERT_OVG_EXT)
        .value("RED",                VK_BLEND_OP_RED_EXT)
        .value("GREEN",              VK_BLEND_OP_GREEN_EXT)
        .value("BLUE",               VK_BLEND_OP_BLUE_EXT)
    ;
