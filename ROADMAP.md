...
- [x] Finish gfx bindings
    ...
    - [x] Finer grained descriptor API:
        - [x] Split layout from pool
        ...
        - [x] Allow specifying stages that use the descriptor instead of just defaulting to ALL, (useful to prevent validation errors for imgui interop)
...
- [ ] Clean examples
    ...
    - [x] Voxels
        ...
        - [x] Depth buffer
        - [x] Mouse interaction
        - [x] transform not working properly, z is flipped, likely vulkan viewport? Debug this with proper x,y,z axis drawn
    - [x] Raytrace
...
- [x] Device features
    ...
    - [x] Functionality that requires extensions should throw if the extension is not enabled:
        - [x] Dynamic rendering
        - [x] Sync2
- [x] If blocked in process_events -> ctrl+c not working
    ...
    - [x] Check if can get interrupt somehow and unblock the loop (e.g. with glfwPostEmptyEvent)
        ...
        - [x] check on linux -> already works, on the pi on waylnad we have to draw once first before wait though
- [x] Input callbacks:
    - [x] Add more keys
...
- [ ] ImGui:
    ...
    - [x] Implement IM_ASSERT to throw exception? or maybe even just std::abort / std::assert, probably easier
        - [x] Avoid asserts based on missing begin / end pairs, either with context manager or with some other
            workaround to avoid them
        - [x] This is already implemented in ImGui, it can also now be set from ImGuiIO which is exposed
    - [x] images interop
    - [x] fonts
    - [x] Expose configuration of .ini file. It's part of ImGuiIO but it requires static strings, so likely need
        some helpers in python here.  (default could be autogenerated from filename or smth, definitely should not be the working directory)
...
- [ ] ImPlot:
    - [x] Likely use generator for enums
    - Not sure if we'll need Range/Rect manually, ideally do most stuff with numpy arrays or tuples/lists
    ...
    - [ ] Setup:
        ...
        - [ ] axis ticks -> numpy array (by hand)
        - [x] axis scale -> no custom transform first, can think about adding about adding later, ideally would have batched api
        - [x] setup finish -> can expose but not that useful I guess? we could make setup methods fields of a setup context manager that calls it at the end
